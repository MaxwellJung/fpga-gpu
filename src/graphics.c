...
void rect(uint16_t x, uint16_t y) {
    for (int rect_y = cp.y; rect_y <= y; ++rect_y) {
        for (int rect_x = cp.x; rect_x <= x; ++rect_x) {
...
void rectR(uint16_t dx, uint16_t dy) {
    for (int rect_y = cp.y; rect_y < cp.y+dy; ++rect_y) {
        for (int rect_x = cp.x; rect_x < cp.x+dx; ++rect_x) {
            writePixel(rect_x, rect_y, color_index);
        }
    }
}
